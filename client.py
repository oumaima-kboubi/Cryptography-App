...
class Client:
    def __init__(self, server, port, username, idd):
        self.server = server
        self.port = port
    ...
    def create_connection(self):
        ...
        self.s.send(self.username.encode('utf-8'))  # Inform the server about the username connected
        ...
        secret_key = self.handle_secret()  # Function to get the secret generated by the server
    ...
    def handle_messages(self):
        while True:
            ...
            if message:
                ...
                iv = b64decode(decrypt_message['iv'])  # Take out the initialization vector and b64 decode it
                cipherText = b64decode(decrypt_message['ciphertext'])  # Take out the ciphertext and b64 decode it
                cipher = AES.new(key, AES.MODE_CFB, iv=iv)  # Create and AES object, parameters: [secret_key], [counter feedback mode], [initialization vector]
                msg = cipher.decrypt(cipherText)  # Use the object to decrypt the ciphertext
                current_time = datetime.datetime.now()
                print(colored(current_time.strftime('%Y-%m-%d %H:%M:%S ') + msg.decode(),  'green'))  # It produces a byte encoded output so you have to decode it to display as string
            else:
                print(colored('[!] Lost the connection to the server', 'red'))
                print(colored('[!] Closing down the connection', 'red'))
                authentification.logout(self.idd)
    ...
    def input_handler(self):
        while True:
            ...
            if message == "EXIT":  # EXIT will close down the client
                ...
                key = secret_key
                cipher = AES.new(key,
                                 AES.MODE_CFB)  # Initialize AES object for encryption, parameters: [key], [counter feedback mode]
                message_to_encrypt = self.username + ": " + message  # The message what will be sent, containing the username and the user input
                msgBytes = message_to_encrypt.encode()  # Byte encode it, because AES input must be byte encoded
                encrypted_message = cipher.encrypt(msgBytes)  # Encrypt the message
                iv = b64encode(cipher.iv).decode(
                    'utf-8')  # Generate the initialization vector, b64encode it, than utf-8 representation to send
                message = b64encode(encrypted_message).decode(
                    'utf-8')  # Same process to the encrypted message, to overcome special chars
                result = json.dumps({'iv': iv, 'ciphertext': message})  # Insert it into a json dictionary
    ...
    def handle_secret(self):
        secret_key = self.s.recv(1024)  # The secret key coming from the server, and used for encryption and decryption
        private_key = RSA.importKey(
            open('client_private_key.pem', 'r').read())  # Import the client private key to decrypt the secret
        cipher = PKCS1_OAEP.new(private_key)  # Using the client private key to decrypt the secret
    ...
    def create_key_pairs(self):
        try:
            ...
            with open('client_private_key.pem', 'w') as priv:  # writing priv key to pem file
                priv.write(private_pem)
            with open('client_public_key.pem', 'w') as pub:  # writing public key to pem file
                pub.write(public_pem)

        except Exception as e:
            print(colored('[!] ERROR, you messed up something.... ' + e, 'red'))

    def menu(cls,username, idd):
        # username = input("Your Username please: ")
        client = Client("localhost", 4444, username, idd)
