...
class Server:
    def __init__(self, port):
        self.host = 'localhost'  # or can be 127.0.0.1
        self.port = port
        os.system('color')
    ...
    def start_server(self):
        ...
        # The arguments passed to socket() specify the address family and socket type. AF_INET is the Internet
        # address family for IPv4. SOCK_STREAM is the socket type for TCP, the protocol that will be used to
        ...
        while True:
            ...
            self.broadcast(colored(' New person joined the room. Username: ' + username, 'yellow'))
    ...
    def handle_client(self, c, addr):
        ...
        while True:
            ...
            if msg.decode() != '':
                ...
                print(colored(current_time.strftime('%Y-%m-%d %H:%M:%S') + ' Message exchanged', 'blue'))
                for connection in self.clients:
                    if connection != c:
                        connection.send(msg)
            else:
                print(colored('[+] ' + self.username_lookup[c] + ' left the server.', 'red'))
...
if __name__ == "__main__":
    # Port needed as argument which will be used to open the socket
   # arg_parse = argparse.ArgumentParser()
    #arg_parse.add_argument("-p", "--port", type=int, required=True, help="port to run the server")
